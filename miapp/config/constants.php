...
if (isset($_SERVER['HTTP_HOST'])) {
    ...
    if ($publicPos !== false) {
        $baseUrlPath = substr($scriptName, 0, $publicPos + 8);
    } else {
        $baseUrlPath = '/';
    }
    define('BASE_URL', $protocol . '://' . $_SERVER['HTTP_HOST'] . $baseUrlPath);
} else {
    define('BASE_URL', 'http://localhost/ING%20WEB%20-%20RECTIFICADORA%20DE%20REPUESTOS/miapp/public/');
}
