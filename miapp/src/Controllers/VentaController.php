    ...
    public function store() {
        ...
        try {
            ...
            if (isset($_POST['items']) && is_array($_POST['items'])) {
                ...
                error_log("POST items: " . print_r($_POST['items'], true));
                
                $items = array_values(array_filter($_POST['items'], function($it){
                    $hasRepuesto = !empty($it['repuesto_id']);
                    $hasCantidad = !empty($it['cantidad']);
                    error_log("Filtering item - repuesto_id: " . ($it['repuesto_id'] ?? 'null') . ", cantidad: " . ($it['cantidad'] ?? 'null') . ", hasRepuesto: " . ($hasRepuesto ? 'true' : 'false') . ", hasCantidad: " . ($hasCantidad ? 'true' : 'false'));
                    return $hasRepuesto && $hasCantidad;
                }));
                
                error_log("Filtered items count: " . count($items));
            }
            $payload = [
