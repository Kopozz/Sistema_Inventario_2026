...
function addItemRow(){
  ...
  tr.innerHTML = `<td>${selectHtml}</td>
    ...
    <td class="fw-bold subtotalCell"><span class="price-amount" data-soles="0.00">S/ 0.00</span></td>
...
function recalcRow(el){
  ...
  const sub = qty * price;
  row.querySelector('.subtotalCell').innerHTML = `<span class="price-amount" data-soles="${sub.toFixed(2)}">S/ ${sub.toFixed(2)}</span>`;
  recalcTotals();
}
...
function recalcTotals(){
  ...
  document.getElementById('subtotalDisplay').innerHTML = `<span class="price-amount" data-soles="${subtotal.toFixed(2)}">S/ ${subtotal.toFixed(2)}</span>`;
  let descuento = parseFloat(document.getElementById('descuento').value||'0');
  if (descuento > subtotal) descuento = subtotal;
  document.getElementById('descuento').value = descuento.toFixed(2);
  const total = subtotal - descuento;
  document.getElementById('totalDisplay').innerHTML = `<span class="price-amount" data-soles="${total.toFixed(2)}">S/ ${total.toFixed(2)}</span>`;
}
